...
import RenderLaw from './RenderLaw';
import {deleteCriminalLaws} from '../firebaseConfig'
const db = firebase.database();
class Criminal_admin extends Component {
    constructor(props) {
        super(props);
        this.state = {
            laws : [],
    ...
    onChange(e){
        ...
        let filterText = this.state.laws.filter((el)=>{
            if(el.offence){
                return el.offence.toLowerCase().indexOf(this.state.searchText.toLowerCase()) !== -1
            }
        });
        this.setState({searchArr : filterText})
    }
    editComponent(el){
        this.props.history.push('/renderLaw',{el : el});
    }

    deleteCriminal(el){
        let del = deleteCriminalLaws(el._id);
        alert('law deleted sucessfully');
        this.getData()
    }

    addLaw(){
        this.props.history.push('/LawAdd');
    }
    ...
    render() {
        ...
        return (
            <div style={{alignItems:'center',justifyContent:'center'}}>
                ...
                <center><div>
                    <button type="button" class="btn btn-success" onClick= {()=>{this.addLaw()}}>Add</button>
                                    ...
                                    <div class="modal-dialog modal-lg">
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-body">
                                                ...
                                                <p><span> <b>Offences : </b> </span> {el.offence}</p>
                                                ...
                                                <button type="button" class="btn btn-success" onClick= {()=>{this.editComponent(el)}}>Edit</button>
                                                <button type="button" class="btn btn-success" onClick= {()=>{this.deleteCriminal(el)}}>Delete</button>
                                                <hr/>
